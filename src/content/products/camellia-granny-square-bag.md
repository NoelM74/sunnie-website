---
name: "Camellia granny square bag"
seoTitle: "Crochet Camellia Granny Square Bag, 3D Flower Drawstring Crossbody"
category: "bags"
group: "flowers"
price: 24.95
inStock: true
etsyUrl: "https://www.etsy.com/shop/SunnieDesignCo"
maker: "Hui"
options: [{"name":"Colour","values":["Red","Green","Pink","Orange"]}]
materials: ["Polyester"]
images: [{"src":"../../assets/products/camellia-granny-square-bag/01.jpg","alt":"Camellia granny square bag"},{"src":"../../assets/products/camellia-granny-square-bag/02.jpg","alt":"Camellia granny square bag, photo 2 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/03.jpg","alt":"Camellia granny square bag, photo 3 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/04.jpg","alt":"Camellia granny square bag, photo 4 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/05.jpg","alt":"Camellia granny square bag, photo 5 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/06.jpg","alt":"Camellia granny square bag, photo 6 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/07.jpg","alt":"Camellia granny square bag, photo 7 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/08.jpg","alt":"Camellia granny square bag, photo 8 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/09.jpg","alt":"Camellia granny square bag, photo 9 of 10"},{"src":"../../assets/products/camellia-granny-square-bag/10.jpg","alt":"Camellia granny square bag, photo 10 of 10"}]
---

A handmade crochet granny square bag with a 3D camellia and matching flower charm on the drawstring. Fits a phone, keys, and daily essentials for hands-free carry.

How to Use/Style:
- Wear it crossbody with a linen dress or prairie skirt for a full cottagecore look.
- The open mesh body works visually against solid-color outfits. Black, cream, and sage all contrast well with the flower accent.
- Available in four flower colors: orange, sage green, pink, and red. Pick the one that matches your usual palette.

Product Details:
- Body: Granny square construction with an open shell-stitch panel in the lower section. Scalloped edge along the top rim.
- Flower: Large 3D crocheted camellia at the center, worked in spiraling layers and hand-attached on a green leaf base.
- Drawstring Charm: A small matching flower bud crocheted onto the drawstring cord, in the same color as the main flower.
- Closure: Green cord drawstring threaded through the scalloped top edge, cinches to close.
- Strap: Braided cream cord, adjustable for crossbody or shoulder carry.
- Capacity: Fits most smartphones alongside a cardholder, lip balm, and keys. Compact daily essentials carry.

Dimensions Top Width 12 cm, Bottom Width 16 cm, Height 15 cm
Measurements may vary 1 to 3 cm. Each bag is crocheted by hand.

-> Color Options: Orange flower, sage green flower, pink flower, red flower. All on a beige cream base.

Care Instructions:
- Hand wash gently with mild soap in cool water.
- Do not wring. Lay flat to dry to keep the granny square shape.
- Do not machine wash or tumble dry.

SHIPPING:
Made and shipped within 2–5 business days. Tracking provided.
All taxes and tariffs are included in the price, so no nasty surprises when your bag arrives.
